...
void _rt_abort ()
{
   ...
   ALERT ("Thread %d: Function abort() was called, but current support for "
         "abort() is very limited",
         TID (__state.current));
   ALERT ("Thread %d: calling pthread_exit() instead of exit(), you might "
         "experience unexpected errors from this point on",
         TID (__state.current));
