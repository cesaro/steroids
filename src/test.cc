...
public:
   ...
   bool find_rt ()
   {
      ld8  = m->getFunction ("stid_rt_load8");
      ld16 = m->getFunction ("stid_rt_load16");
      ld32 = m->getFunction ("stid_rt_load32");
      ld64 = m->getFunction ("stid_rt_load64");

      st8  = m->getFunction ("stid_rt_store8");
      st16 = m->getFunction ("stid_rt_store16");
      st32 = m->getFunction ("stid_rt_store32");
      st64 = m->getFunction ("stid_rt_store64");

      allo = m->getFunction ("stid_rt_alloca");
      mllo = m->getFunction ("stid_rt_malloc");
      ret  = m->getFunction ("stid_rt_ret");
      start= m->getFunction ("stid_rt_start");
   ...
   {
      return f->getName().startswith ("stid_rt_");
   }
   ...
   bool instrument (llvm::Module &m)
   {
      ...
      for (auto &f : m)
      {
         ...
         llvm::outs() << "stid: fe3: instrumenting function '" << f.getName() << "'\n";
         count = 0;
         visit (f);
         llvm::outs() << "stid: fe3: done, " << count << " instructions.\n";

         for (auto i : f.users())
         {
            llvm::outs() << "stid: fe3:  used: " << *i << "\n";
         }
      }
...
void test2 ()
{
   ...
   // execute it
   ee->finalizeObject ();
   void * ptr = (void *) ee->getFunctionAddress ("stid_rt_start");
   ...
   //DEBUG ("stid: entry returned %d %d", ret1, ret2);
   DEBUG ("stid: entry returned %d", ret1);
}
